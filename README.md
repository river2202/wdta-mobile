...
**选择与记忆**
- 两步落地页：先选 Competition（如 Saturday AM），再选 Section，点击即进入结果页。
- 支持**全部 competition / section**（从 TROLS 动态发现，无硬编码）。
- 记住上次选择，下次打开直接进入对应结果页；结果页有"切换"图标可清除选择、回落地页重选。

**赛果展示**
- 当前积分榜（Current ladder）：排名、积分、百分比、场地备注、晋级分割线。
- 比赛按 **Round 折叠**：最新一轮默认展开、其余默认折叠，最新在上，点标题即可展开/收起。
- 每场比赛卡片：比分、R/S/G 统计，以及可展开的比赛详情（双方出场名单、紧急替补标记、各 rubber 比分）。
- bye / wash out / forfeit 等非常规状态单独样式展示。

**深链到 TROLS 源站**
- "Original WDTA"：跳到源站对应 section 的赛果页。
- "Original ladder"：跳到该 section 的积分榜。
- 积分榜每支球队一个 **Fixture** 链接：深链到该队在 fixture.php 的赛程（competition + section + team）。

**数据新鲜度**
- 打开结果页时若缓存超过 2 小时，自动在后台静默刷新。
- 源站赛果更新时间变化会提示"New results available"，可标记为已看。
- 每日 Vercel Cron 在后台分批刷新所有 section，保持缓存温热（约 2 天轮转覆盖全部）。

**其他**
- 手机优先布局，深链均在新标签页打开。
- 可选的 Buy Me a Coffee 捐赠入口（设了 `NEXT_PUBLIC_BUYMEACOFFEE_URL` 才显示）。

**不做**：登录、编辑、球队管理、赛果提交。

> 技术栈：Next.js App Router + TypeScript + Neon Postgres + Vercel（Cron + 托管）。
